...
const Review = () => {
    const { data: reviews, isLoading } = useQuery('reviews', () => fetch(`https://young-gorge-13678.herokuapp.com/review`)
        .then(res => res.json()))
    ...
    return (
        <div className='py-10'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-5 lg:grid-cols-3 justify-center lg:px-20 '>
                {
                    reviews.slice(0, 6).map(review => <ReviewCard review={review} key={review._id} />)
                }
            </div>
